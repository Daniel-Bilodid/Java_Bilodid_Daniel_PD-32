...
public class TaskTwo {
    ...
    public static void main(String[] args) {    // создаение публичного меттода 
        ...
        int size = (new Scanner(System.in)).nextInt(); // size берет данные которые исходят из сканнирование того что написал пользователь  
        int[] arr = new int[size]; //массив
        fillArrayRandomValues(arr, 0, 9); //заполняем массив рандом числами 
        printArray(arr); 

        OptionalDouble result = Arrays.stream(arr) //OptionalDouble - активация тру фолс через дабл  стрим преозбравоние в поток 
